...
/*MOVEING THROUGH THE LIST OF ASTROLOGICAL EVENTS*/
...
/*ISOLATE INFORMATION FROM API*/
//Get individual numbers for the clock
...
//Reorganize data to make it more manageable
...
/*CALCULATE THE OPPOSITE SIDE OF EARTH*/
...
async function mainEvent() {
  ...
  const link =
    "https://api.sunrise-sunset.org/json?lat=" + in_lat + "&lng=" + in_long; //get link
  const in_results = await fetch(link);
  ...
  const op_link =
    "https://api.sunrise-sunset.org/json?lat=" + op_lat + "&lng=" + op_long; //get link
  const op_results = await fetch(op_link);
